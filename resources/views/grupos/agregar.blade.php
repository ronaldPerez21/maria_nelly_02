            ...
            <div class="col-lg-12">
                <div class="ibox ">
                    ...
                    <div class="ibox-content">
                        <form action="{{route("grupos.insertar")}}" method="post">
                            @csrf
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <input type="number" class="form-control" name="cupos" value="{{old('cupos')}}" required="">
                                </div>
                            </div>
                            @error('cupos')
                            <div class="alert alert-danger alert-dismissable">{{$message}}<button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button></div>
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" name="gestion_id"  id="gestion_id">
                                        ...
                                        @foreach ($gestiones as $gestion)
                                            <option value="{{$gestion->id}}" @if(old('gestion_id')==$gestion->id) selected="" @endif  >{{$gestion->anio}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" name="turno_id"  id="turno_id">
                                        ...
                                        @foreach ($turnos as $turno)
                                            <option value="{{$turno->id}}" @if(old('turno_id')==$turno->id) selected="" @endif>{{$turno->nombre}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
