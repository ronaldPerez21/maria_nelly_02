        ...
        <div class="col-lg-12">
            <div class="ibox ">
                <div class="ibox-title">
                    <a class="btn btn-primary" href="{{route('horas.agregar')}}">Agregar</a>
                    <div class="ibox-tools"><a class="collapse-link"><i class="fa fa-chevron-up"></i></a></div>
                </div>
                <div class="ibox-content">
                    <form name="formBuscar" action="{{route("horas.index")}}" method="get">
                        <div class="row">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach($horas as $hora)
                                <tr>
                                    <td>{{$hora->id}}</td>
                                    <td>{{hora_minuto($hora->hora_ini)}}</td>                                    
                                    <td>{{hora_minuto($hora->hora_fin)}}</td>
                                    <td>
                                        @if ($hora->activo) 
                                    ...
                                    <td>{{fecha_latina($hora->created_at) }}</td>
                                    <td data-texto="{{$hora->hora_ini}}">
                                        <a href="{{route('horas.mostrar',$hora->id)}}" title="Mostrar"><img width="17px" src="{{asset('img/iconos/mostrar.png')}}" alt="Mostrar"></a>
                                        <a href="{{route('horas.modificar',$hora->id)}}" title="Modificar"><img width="17px" src="{{asset('img/iconos/modificar.png')}}" alt="Modificar"></a>
                                        <a data-ruta="{{route('horas.eliminar',$hora->id)}}" class="btn-eliminar" title="Eliminar"><img width="17px" src="{{asset('img/iconos/eliminar.png')}}" alt="Eliminar"></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
