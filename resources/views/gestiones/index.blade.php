        ...
        <div class="col-lg-12">
            <div class="ibox ">
                <div class="ibox-title">
                    <a class="btn btn-primary" href="{{route('gestiones.agregar')}}">Agregar</a>
                    <div class="ibox-tools"><a class="collapse-link"><i class="fa fa-chevron-up"></i></a></div>
                </div>
                <div class="ibox-content">
                    <form name="formBuscar" action="{{route("gestiones.index")}}" method="get">
                        <div class="row">
                    ...
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                <th>Id</th>
                                <th>Año</th>
                                <th>Fecha Inico clases</th>
                                <th>Fecha Final clases</th>
                                <th>Numero periodos</th>
                                <th>Tipos periodos</th>
                                <th>Activo</th>
                                <th>Creado</th>
                                <th>&nbsp;</th>
                            </tr>
                        ...
                        <tbody>
                            @foreach($gestiones as $gestion)
                                <tr>
                                    <td>{{$gestion->id}}</td>
                                    <td>{{$gestion->anio}}</td>
                                    <td>{{($gestion->fecha_inicio_clases)}}</td>                                    
                                    <td>{{($gestion->fecha_final_clases)}}</td>
                                    <td>{{($gestion->numeros_periodos)}}</td>
                                    <td>{{($gestion->tipos_periodos)}}</td>
                                    <td>
                                        @if ($gestion->activo) 
                        ...
                        <script>
                            $(document).ready(function(){
                                $('.btn-eliminar').click(function(){
                                    ...
                                    if(esEliminar){
                                        $('#formEliminar').attr('action',gestion);
